...
// Update check disabled — this is a standalone fork
const GITHUB_OWNER = '';
const GITHUB_REPO = '';
...
if (process.platform === 'win32') {
  configPath = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'oled-claude-usage-monitor', 'config.json');
} else if (process.platform === 'darwin') {
  configPath = path.join(os.homedir(), 'Library', 'Application Support', 'oled-claude-usage-monitor', 'config.json');
} else {
  configPath = path.join(os.homedir(), '.config', 'oled-claude-usage-monitor', 'config.json');
}
...
function createMainWindow() {
  ...
  const savedOpacity = store.get('settings.windowOpacity', 100);
  if (savedOpacity < 100) {
    mainWindow.setOpacity(Math.max(0.2, savedOpacity / 100));
  }
...
function createTray() {
  try {
    ...
    tray.setToolTip('OLED Claude Usage Monitor');
...
ipcMain.on('set-window-opacity', (event, opacity) => {
  if (mainWindow) {
    const clamped = Math.max(0.2, Math.min(1, parseFloat(opacity) || 1));
...
ipcMain.handle('get-settings', () => {
  return {
    ...
    windowOpacity: store.get('settings.windowOpacity', 100),
    glassMode: store.get('settings.glassMode', false),
    pixelShift: store.get('settings.pixelShift', false),
    sessionAccent: store.get('settings.sessionAccent', 'purple'),
    weeklyAccent: store.get('settings.weeklyAccent', 'blue'),
    glassHoverOpacity: store.get('settings.glassHoverOpacity', 60),
    showSession: store.get('settings.showSession', true),
    showWeekly: store.get('settings.showWeekly', true),
    showElapsed: store.get('settings.showElapsed', true),
    showResetsIn: store.get('settings.showResetsIn', true),
    showResetsAt: store.get('settings.showResetsAt', true),
    showLogo: store.get('settings.showLogo', false)
  };
});
...
ipcMain.handle('save-settings', (event, settings) => {
  ...
  store.set('settings.windowOpacity', settings.windowOpacity);
  store.set('settings.glassMode', settings.glassMode);
  store.set('settings.pixelShift', settings.pixelShift);
  store.set('settings.sessionAccent', settings.sessionAccent);
  store.set('settings.weeklyAccent', settings.weeklyAccent);
  store.set('settings.glassHoverOpacity', settings.glassHoverOpacity);
  store.set('settings.showSession', settings.showSession);
  store.set('settings.showWeekly', settings.showWeekly);
  store.set('settings.showElapsed', settings.showElapsed);
  store.set('settings.showResetsIn', settings.showResetsIn);
  store.set('settings.showResetsAt', settings.showResetsAt);
  store.set('settings.showLogo', settings.showLogo);
...
// Check GitHub releases for a newer version (disabled in standalone fork)
ipcMain.handle('check-for-update', () => {
  if (!GITHUB_OWNER || !GITHUB_REPO) return Promise.resolve({ hasUpdate: false, version: null });
  return new Promise((resolve) => {
    const options = {
      ...
      method: 'GET',
      headers: {
        'User-Agent': 'oled-claude-usage-monitor',
