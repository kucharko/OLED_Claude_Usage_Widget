# Credits

This application is based on **Claude Usage Widget** by **Slavomir Durej**.

- Original repository: https://github.com/SlavomirDurej/claude-usage-widget
- License: MIT
- Original author: Slavomir Durej (https://github.com/SlavomirDurej)

The original project was open-sourced under the MIT license, which permits
modification, distribution, and private use.

## What changed

This fork — **OLED Claude Usage Monitor Widget** — adds:
- OLED display mode with pure black backgrounds and burn-in prevention
- Window opacity control
- Visual UI/UX improvements (gradients, micro-interactions, polish)
- Windows-only simplified build
